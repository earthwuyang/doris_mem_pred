...
def parse_physical_node(node_str, node_level, node_id):
   ...
   #  node_pattern = r'([A-Za-z0-9]+)(?:\[([0-9]+)\])?(?:@([0-9]+)?)? \(([^)]+)\)'  # CAUTION that for tpcds, this node_pattern is different from below pattern, in that ?:@( ?:[0-9]+)
    ...
    node = Node(node_id, name, int(id_) if id_ else -1, int(order) if order else -1, attributes, int(float(cardinality)) if cardinality else 0, table, node_level)  
...
def parse_tree(text):
    ...
    edge_tgt_nodes=[]
    features_list = []
    for node_id, line in enumerate(texts):
        # print(f"line: [{line}]")
        ...
        if stack:
            ...
            edge_src_nodes.append(node.nodeid)
            edge_tgt_nodes.append(stack[-1].nodeid)
            # print(node.children)

        stack.append(node)


    return stack[0], edge_src_nodes, edge_tgt_nodes, features_list    # stack[0] is the root node
...
input_str = """
PhysicalResultSink[164] ( outputExprs=[id#24, catalog_id#25, db_id#26, tbl_id#27, idx_id#28, col_id#29, part_id#30, row_count#31, ndv#32, null_count#33, min#34, max#35, data_size#36, update_time#37] )
+--PhysicalProject[161]@3 ( stats=1, projects=[concat('25982', '-', '-1', '-', 'wr_account_credit') AS `id`#24, 0 AS `catalog_id`#25, 23644 AS `db_id`#26, 25982 AS `tbl_id`#27, -1 AS `idx_id`#28, 'wr_account_credit' AS `col_id`#29, NULL AS `part_id`#30, row_count#31, ndv#32, (row_count#31 - count(wr_account_credit)#38) AS `null_count`#33, substring(cast(min(wr_account_credit)#39 as TEXT), 1, 1024) AS `min`#34, substring(cast(max(wr_account_credit)#40 as TEXT), 1, 1024) AS `max`#35, (row_count#31 * 4) AS `data_size`#36, 2024-07-18 07:35:40 AS `update_time`#37] )
   +--PhysicalHashAggregate[158]@2 ( aggPhase=GLOBAL, aggMode=BUFFER_TO_RESULT, maybeUseStreaming=false, groupByExpr=[], outputExpr=[count(partial_count(*)#41) AS `row_count`#31, ndv(partial_ndv(wr_account_credit)#42) AS `ndv`#32, count(partial_count(wr_account_credit)#43) AS `count(wr_account_credit)`#38, min(partial_min(wr_account_credit)#44) AS `min(wr_account_credit)`#39, max(partial_max(wr_account_credit)#45) AS `max(wr_account_credit)`#40], partitionExpr=Optional[[]], requireProperties=[GATHER], topnOpt=false, stats=1 )
      +--PhysicalDistribute[155]@5 ( stats=1, distributionSpec=DistributionSpecGather )
         +--PhysicalHashAggregate[152]@5 ( aggPhase=LOCAL, aggMode=INPUT_TO_BUFFER, maybeUseStreaming=false, groupByExpr=[], outputExpr=[partial_count(*) AS `partial_count(*)`#41, partial_ndv(wr_account_credit#22) AS `partial_ndv(wr_account_credit)`#42, partial_count(wr_account_credit#22) AS `partial_count(wr_account_credit)`#43, partial_min(wr_account_credit#22) AS `partial_min(wr_account_credit)`#44, partial_max(wr_account_credit#22) AS `partial_max(wr_account_credit)`#45], partitionExpr=Optional[[]], requireProperties=[ANY], topnOpt=false, stats=1 )
            +--PhysicalProject[149]@1 ( stats=21,593,010, projects=[wr_account_credit#22] )
               +--PhysicalOlapScan[web_returns]@0 ( stats=21,593,010 )
"""
...
if __name__ == '__main__':
   # Parse the tree
   root_node, _, _, _ = parse_tree(input_str)
